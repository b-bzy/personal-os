...
**最常见形态是语音听写转成的文字**：一整段口语、没有分点、可能夹着识别错字，「背景 / 问题 / 经验」三部分混在一起讲。直接按 A2 拆开，**不要因为用户没分点就反问他要格式**。

其他形态：手打文字 / 截图 / PDF。图片用 `Read` 工具直接读，不要跑 OCR。
...
- ✅ 语音听写的同音错字按上下文纠正（"弹窗"误听成"弹床"之类）
- ❌ 不合并/拆分用户分点数、不增加用户没写的内容、不脑补根因
- ⚠️ **数字和人名不要猜**：听写里的数字、人名、专有名词如果明显可疑，入库前问一句，不要按上下文替他改

### A2.6 · experience 必须是"下次的动作"
用户说成感想（"以后要多沟通"）时，追问一句"具体下次怎么做"，或按他原话里已有的动作线索改写成可验证的动作。
**判断标准：一周内能验证做没做到。** 用户确实说不出来就留空，不要替他编。
